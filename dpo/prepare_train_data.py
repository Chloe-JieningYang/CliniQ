...
def mode_model(
    ...
    # 2. Load base model to CPU first to avoid accelerate get_balanced_memory bug
    print(f"Loading base model: {base_model_id}")
    base_model = AutoModelForCausalLM.from_pretrained(
        base_model_id,
        torch_dtype=torch.float16,
        device_map="auto",           # CPU first — adapter merge must happen before dispatch
        trust_remote_code=True,
    )

    # 3. Attach SFT adapter, merge, then dispatch to GPU
...
def mode_model_rejected(
    ...
    # 2. Load base model
    print(f"Loading base model: {base_model_id}")
    # FIX: load to CPU first (device_map=None), then attach adapter, then dispatch.
    # Using device_map="auto" here triggers accelerate's get_balanced_memory which
    # has a bug with sets in older versions: TypeError: unhashable type: 'set'.
    base_model = AutoModelForCausalLM.from_pretrained(
        base_model_id,
        torch_dtype=torch.float16,
        device_map="auto",           # CPU first — adapter merge must happen before dispatch
    ...
    # 3. Load SFT adapter, merge, then dispatch to GPU
    print(f"Loading and merging SFT adapter from: {model_path}")
    model = PeftModel.from_pretrained(base_model, model_path, is_trainable=False)
    model = model.merge_and_unload()   # plain nn.Module — safe to dispatch now
